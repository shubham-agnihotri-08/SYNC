  ...
  const fetchSickLeaves = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const sickOnly = (data.leaveRequests || []).filter((req: LeaveRequest) => req.type === "SICK")
        setSickLeaves(sickOnly)
      }
    } catch (error) {
  ...
  const handleAction = async (id: string, status: "APPROVED" | "REJECTED") => {
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: `Sick leave ${status.toLowerCase()}`,
        })
        fetchSickLeaves()
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to update sick leave",
        variant: "destructive",
      })
    }
  }

  const filteredLeaves = sickLeaves.filter((req) => req.user.name.toLowerCase().includes(searchQuery.toLowerCase()))
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Sick Leaves</h1>
          <p className="text-muted-foreground">View and manage all sick leave requests</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-4">
            <CardTitle>All Sick Leaves</CardTitle>
        ...
        <CardContent>
          <div className="space-y-4">
            ...
            {filteredLeaves.length === 0 && (
              <div className="py-12 text-center text-muted-foreground">No sick leave requests found</div>
            )}
          </div>
        </CardContent>
      </Card>
